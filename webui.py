...
def run_train_index(
    ...
    print(
        "INDEX DEBUG:",
        "exp_dir1 =", repr(exp_dir1),
        "version19 =", repr(version19),
        "training_mode =", repr(training_mode),
        "CWD =", repr(now_dir),
    )
...
with gr.Blocks(title="RVC WebUI", css=TRAINING_INFO_CSS) as app:
    ...
    gr.Markdown(
        "Interfaz simplificada para uso personal. El backend original de RVC se conserva."
    )
    ...
    with gr.Tabs():
        ...
        # 1. CONVERTIR VOZ
        # ==============================================================
        with gr.TabItem("🔊 Convertir voz"):
            gr.Markdown(
                "Selecciona un modelo `.pth` y un audio de entrada. "
                "Los modelos hablados normalmente no usan F0; los modelos de canto "
                "deben estar entrenados con F0. El backend determina la arquitectura "
                "real del modelo cargado."
            )
            ...
            with gr.Row():
                sid0 = gr.Dropdown(
                    ...
                    value=sorted(names)[0] if names else None,
                    interactive=True,
                    scale=4,
                )
                with gr.Column(scale=1, min_width=130):
                    refresh_button = gr.Button("Actualizar modelos")
                    clean_button = gr.Button("Liberar modelo")

            # El backend de RVC obtiene del .pth si el modelo usa F0 y su
            # configuración real. Este selector sirve como guía para el usuario.
            inference_type = gr.Radio(
                label="Tipo de modelo",
                choices=["Voz hablada", "Canto"],
                value="Voz hablada",
                interactive=True,
            )
            ...
            with gr.Row():
                ...
                with gr.Column():
                    vc_transform0 = gr.Number(
                        label="Ajuste de tono (semitonos)",
                        value=0,
                        precision=0,
                        interactive=True,
                    )
                    f0method0 = gr.Radio(
                        label="Método F0 (para canto)",
                        choices=["rmvpe"],
                        value="rmvpe",
                        interactive=True,
                    )

            # Parámetros técnicos necesarios por VC.vc_single. Se mantienen
            # ocultos para no complicar la interfaz.
            spk_item = gr.Slider(
                minimum=0,
                maximum=2333,
                step=1,
            ...
            # Variables que VC.get_vc actualiza también para la inferencia
            # por lotes se conservan aunque la interfaz por lotes se haya
            # eliminado.
            file_index3 = gr.Textbox(value="", visible=False, interactive=True)
            protect1 = gr.Slider(
                minimum=0,
                maximum=0.5,
                step=0.01,
            ...
            with gr.Row():
                ...
                vc_output2 = gr.Audio(label="Audio de resultado")
            ...
            # Se conserva exactamente el mecanismo existente para cargar el
            # modelo, seleccionar speaker IDs y encontrar automáticamente el índice.
            sid0.change(
                fn=vc.get_vc,
        ...
        # 2. ENTRENAR VOZ
        # ==============================================================
        with gr.TabItem("🎙️ Entrenar voz"):
            gr.Markdown(
                "Entrenamiento simplificado. El botón ejecuta el flujo existente "
                "de RVC: preprocesado → F0/HuBERT → entrenamiento → índice."
            )
            ...
            with gr.Row():
                ...
                trainset_dir4 = gr.Textbox(
                    ...
                    placeholder=r"C:\ruta\al\dataset",
                    interactive=True,
                    scale=3,
                )
            ...
            with gr.Row():
                ...
                gpu_choices = [str(i) for i in gpu_indices] or [""]
                default_gpu = str(gpu_indices[0]) if gpu_indices else ""
                gpus16 = gr.Dropdown(
                    label="GPU",
                    choices=gpu_choices,
                    value=default_gpu,
                    interactive=bool(gpu_indices),
                )

            # Valores técnicos fijados según la configuración solicitada.
            # training_type devuelve 0 para hablado y 1 para canto, por lo
            # que puede utilizarse directamente como if_f0_3 sin crear una
            # función adaptadora nueva.
            sr2 = gr.State("40k")
            version19 = gr.State("v2")
            if_f0_3 = training_type
            spk_id5 = gr.State(0)
            np7 = gr.State(int(np.ceil(config.n_cpu / 1.5)))
            f0method8 = gr.State("rmvpe")
            save_epoch10 = gr.State(10)
            if_save_latest13 = gr.State(i18n("是"))
            if_cache_gpu17 = gr.State(i18n("否"))
            if_save_every_weights18 = gr.State(i18n("否"))
            gpus_rmvpe = gpus16

            pretrained_G14 = gr.Textbox(
                value="assets/pretrained_v2/G40k.pth",
                visible=False,
                interactive=False,
            )
            pretrained_D15 = gr.Textbox(
                value="assets/pretrained_v2/D40k.pth",
                visible=False,
                interactive=False,
            )

            # change_sr2 ya existe en el backend y selecciona automáticamente
            # G40k/D40k o f0G40k/f0D40k según if_f0_3, manteniendo v2 + 40k.
            training_type.change(
                change_sr2,
                [sr2, training_type, version19],
                [pretrained_G14, pretrained_D15],
                queue=False,
            )

            gr.Markdown(
                "Configuración automática: 40 kHz · v2 · "
                "hablado sin F0 · canto con F0 + RMVPE."
            )

            with gr.Row():
                train_button = gr.Button("🎙️ Entrenar", variant="primary")
            ...
            # train1key conserva el flujo real completo y sus argumentos.
            train_button.click(
                train1key,
                [
                    exp_dir1,
                ...
                [train_status, train_button, stop_train_button, sid0],
        ...
        with gr.TabItem("👥 Multispeaker"):
            gr.Markdown(
                "Asistente original para crear y enviar el manifest de "
                "entrenamiento multispeaker. Se conservan speakers, IDs, "
                "rutas, repetición, paginación, manifest y sincronización "
            ...
            with gr.Row():
                helper_exp_name = gr.Textbox(
                    ...
                    placeholder="ej. multispeaker_test",
                    interactive=True,
                )
                helper_submit = gr.Button(
                    "Enviar manifest",
                    variant="primary",
                )
                helper_previous = gr.Button("← Anterior")
                helper_page_label = gr.Markdown(
                    value="Página 1/1 · 2 filas"
                )
                helper_next = gr.Button("Siguiente →")

            with gr.Row():
                helper_add = gr.Button("➕ Agregar speaker")
                helper_remove = gr.Button("➖ Eliminar speaker")
            ...
            # Se mantiene la sincronización bidireccional original entre el
            # nombre del experimento y el nombre usado por el helper.
            exp_dir1.change(
                sync_exp_name,
